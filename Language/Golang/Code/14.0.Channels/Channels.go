...
// passa channel to the function
// send message with that function to the main function

func testingChannel(ch chan<- string) {
	time.Sleep(time.Second * 5)
	mess := "First Message"
	ch <- mess
	// fmt.Println("Sending message from testing channel")
	time.Sleep(time.Second * 2)
