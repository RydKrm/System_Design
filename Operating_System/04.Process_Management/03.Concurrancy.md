...
Solution 1:
 Asymmetric (one phil different): Philosopher 4 picks up RIGHT fork first Breaks circular wait

Solution 2: 
  All or nothing: 
  while (1) {
   lock_both_forks_atomically();
   eat(); 
   unlock_both_forks();
}

Solution 3: 
 Waiter (central arbiter): 
 while (1) { 
   ask_waiter_permission(); // Only 4 can eat at once pick_up_forks(); 
   eat(); 
   put_down_forks(); 
   release_permission(); 
}
...
## Real-World Example: Web Server Concurrency

Let's see how a real web server handles concurrent requests:
# WEB SERVER: HANDLING CONCURRENT REQUESTS

Scenario: Apache/Nginx-style web server
...
Main Thread: while (1) { client_socket = accept(server_socket); add_to_queue(client_socket); }

Worker Threads (100 of them): while (1) { client_socket = get_from_queue(); // Blocks if empty handle_request(client_socket); }

REQUEST HANDLING: ─────────────────

void handle_request(int client_socket) { // 1. Read HTTP request char buffer[4096]; read(client_socket, buffer, 4096);

```
// 2. Parse request
char* filename = parse_url(buffer);

// 3. Check cache (SHARED DATA!)
pthread_mutex_lock(&cache_lock);
cached_file = cache_lookup(filename);
if (cached_file) {
    pthread_mutex_unlock(&cache_lock);
    send(client_socket, cached_file, ...);
    return;
}
pthread_mutex_unlock(&cache_lock);

// 4. Read file from disk (SLOW! May block)
char* file_content = read_file(filename);

// 5. Update cache (SHARED DATA!)
pthread_mutex_lock(&cache_lock);
cache_insert(filename, file_content);
pthread_mutex_unlock(&cache_lock);

// 6. Send response
send(client_socket, file_content, ...);
close(client_socket);
}
```
...
100 worker threads grab requests:

Core 0: [Thread 1: R1] [Thread 2: R2] [Thread 3: R3] ...
Core 1: [Thread 26: R26] [Thread 27: R27] ...
Core 2: [Thread 51: R51] [Thread 52: R52] ...
Core 3: [Thread 76: R76] [Thread 77: R77] ...

Time: 10ms - Threads start accessing shared cache
──────────────────────────────────────────────────

Thread 1: Locks cache, checks for "index.html"
Thread 2: Tries to lock cache → BLOCKS
Thread 3: Tries to lock cache → BLOCKS
...

Thread 1: Unlocks cache (miss, file not cached)
Thread 2: Acquires lock, checks for "style.css"
...

Time: 15ms - Multiple threads reading files
────────────────────────────────────────────

Thread 1: read_file("index.html") → BLOCKS (disk I/O)
Thread 5: read_file("logo.png") → BLOCKS (disk I/O)
Thread 12: read_file("script.js") → BLOCKS (disk I/O)

Scheduler context-switches to other threads:
Thread 2: Processing different request
Thread 3: Processing different request
...

Time: 20ms - Some reads complete
─────────────────────────────────

Thread 1: Disk I/O done, wakes up
Thread 1: Locks cache, inserts "index.html"
Thread 1: Unlocks cache
Thread 1: Sends response to client
Thread 1: Gets next request from queue

Time: 50ms - Cache is warm
───────────────────────────

Many subsequent requests hit cache:
Thread 45: Locks cache
Thread 45: Finds "index.html" (HIT!)
Thread 45: Unlocks cache
Thread 45: Sends cached content (FAST!)
...
1. RACE CONDITION on cache:
   ────────────────────────
   Thread A: Checks cache → MISS
   Thread B: Checks cache → MISS
   Thread A: Reads file, inserts to cache
   Thread B: Reads same file, inserts to cache
   Result: Wasted disk read, duplicate cache entry
   
   Fix: Double-check locking, or mark "loading" state

2. DEADLOCK with multiple caches:
   ───────────────────────────────
   Thread 1: Lock file_cache → Lock stats_cache
   Thread 2: Lock stats_cache → Lock file_cache
   Result: Deadlock
...
3. THUNDERING HERD:
   ────────────────
   1000 threads all wake up when request arrives
   All try to grab lock simultaneously
   99.9% immediately block again
   Wasted CPU time
   
   Fix: Use condition variables properly

4. CACHE CONTENTION:
   ─────────────────
   Cache lock becomes bottleneck
   All threads serialize at cache access
   4 cores, but only 1 thread in cache at a time
   
   Fix: Shard cache (multiple independent caches)
...
Without threading (sequential):
  1000 requests × 50ms each = 50,000ms = 50 seconds

With 100 threads (4 cores):
  - Some parallelism: ~4x speedup from cores
  - Thread reuse: No process creation overhead
  - Overlap I/O with computation
  - Result: ~3-5 seconds for all requests
  
Speedup: ~10-15x!

But:
  - Lock contention limits scalability
  - Context switching overhead (~2% CPU)
  - Cache bouncing between cores
  - Diminishing returns beyond ~CPU_cores × 2 threads
```
...
Concurrency is essential for modern computing, enabling systems to efficiently utilize resources and remain responsive. However, it introduces significant challenges:

**Benefits of Concurrency:**
- Better CPU utilization (overlap I/O with computation)
- Improved responsiveness (UI remains interactive)
- Higher throughput (handle more requests per second)
- Resource sharing (threads share memory efficiently)

**Problems Created:**
1. **Race Conditions**: Unpredictable results from simultaneous access
2. **Deadlocks**: Circular waiting prevents all progress
3. **Starvation**: Some threads never get resources
4. **Data Corruption**: Inconsistent state from partial updates

**Solutions:**
1. **Mutexes/Locks**: Serialize access to critical sections
2. **Semaphores**: Control access to resource pools
3. **Atomic Operations**: Lock-free, fast operations for simple cases
4. **Proper Design**: Lock ordering, timeouts, avoiding shared state when possible

**Process vs Thread Concurrency:**
- **Processes**: Isolated memory (safe but expensive communication via IPC)
- **Threads**: Shared memory (fast communication but requires careful synchronization)

The key to successful concurrent programming is understanding these trade-offs and choosing the right synchronization primitives for your use case. Modern systems rely heavily on concurrency—from web servers handling thousands of connections to video players decoding multiple streams—making these concepts fundamental to computer science.